...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    intern=torch.matmul(query,key)/sqrt(self.attention_head_size)+attention_mask
    intern=F.softmax(intern,dim=3)
    final=torch.matmul(intern,value).transpose(1,2)
    final=torch.reshape(final,(final.shape[0],final.shape[1],self.all_head_size))
    #reshape更好用，view会导致共享数据，需要内存的连续存储，会报错
    return final
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    output_drop=dropout(output_trans)
    return ln_layer(input+output_drop)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    ff_input=self.add_norm(hidden_states,hidden_states_out,self.attention_dense,
                           self.attention_dropout,self.attention_layer_norm)
    ff_internout=self.interm_af(self.interm_dense(ff_input))
    return self.add_norm(ff_input,ff_internout,self.out_dense,self.out_dropout,self.out_layer_norm)
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    ### TODO
    final_input=inputs_embeds+pos_embeds+tk_type_embeds
    final_input=self.embed_layer_norm(final_input)
    return self.embed_dropout(final_input)
