...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.outproject_simi2=torch.nn.Linear(config.hidden_size,config.hidden_size)
    ...
    def predict_similarity(self,
        ...
        ### TODO
        pooler_output_1=self.forward(input_ids_1,attention_mask_1)
        pooler_output_1=F.tanh(self.outproject_simi1(pooler_output_1))
        pooler_output_1=torch.unsqueeze(pooler_output_1,dim=1)
        pooler_output_2=self.forward(input_ids_2,attention_mask_2)
        pooler_output_2=F.tanh(self.outproject_simi2(pooler_output_2))
...
def train_multitask(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step, batch in enumerate(tqdm(sts_train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE)):
            ...
            #原因：模型处于pretrain模式同时simi任务当时未接上任何线性层，系统无可更新项
            #解决：loss.requires_grad_(True)强行覆盖同时引入线性层
            loss = F.mse_loss(logits, b_labels.float(), reduction='sum') / args.batch_size
            loss.requires_grad_(True)

            #为打印参数验证模型是否更新，梯度应该在backwatd之前打印，否则重置为0
        ...
        dev_acc=sentiment_accuracy+paraphrase_accuracy+sts_corr
