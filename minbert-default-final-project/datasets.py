...
class SentencePairDataset(Dataset):
    ...
    def pad_data(self, data):
        ...
        '''
        encoding1['input_ids']=[i.tolist() for i in encoding1['input_ids']]
        encoding2['input_ids']=[i.tolist() for i in encoding2['input_ids']]

        token_ids=[self.tokenizer.build_inputs_with_special_tokens(i,j) for i,j in zip(encoding1['input_ids'],encoding2['input_ids'])]
        attention_mask=torch.LongTensor([self.tokenizer.get_special_tokens_mask(i,already_has_special_tokens=True) for i in token_ids])
        token_type_ids=torch.LongTensor([self.tokenizer.create_token_type_ids_from_sequences(i) for i in token_ids])
...
class SentencePairTestDataset(Dataset):
    ...
    def pad_data(self, data):
        ...
        encoding2 = self.tokenizer(sent2, return_tensors='pt', padding=True, truncation=True)
        '''
        token_ids=self.tokenizer.build_inputs_with_special_tokens(encoding1['input_ids'],encoding2['input_ids'])
        attention_mask=torch.LongTensor(self.tokenizer.get_special_tokens_mask(token_ids,already_has_special_tokens=True))
        token_type_ids=torch.LongTensor(self.tokenizer.create_token_type_ids_from_sequences(token_ids))
        token_ids=torch.LongTensor(token_ids)
        '''
        token_ids = torch.LongTensor(encoding1['input_ids'])
        attention_mask = torch.LongTensor(encoding1['attention_mask'])
